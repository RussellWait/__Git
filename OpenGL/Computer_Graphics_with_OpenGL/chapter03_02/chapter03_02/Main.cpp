...
void displayFcn(void)
{
    ...
    std::cout << "one of the following curve names.\n";
    std::cout << "Press any other key to exit.\n";
    std::cout << "\n1-limacon, 2-cardioid, 3-threeLeaf, 4-fourLeaf, 5-spiral: ";
    std::cin >> curveNum;

    if (1 == curveNum || 2 == curveNum || 3 == curveNum || 4 == curveNum || 5 == curveNum)
    {
        drawCurve(curveNum);
    }
    else
...
void winReshapeFccn(GLint newWidth, GLint newHeight)
{
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
...
void main(int argc, char **argv)
{
    ...
    glutReshapeFunc(winReshapeFccn);
    glutMainLoop();
}
